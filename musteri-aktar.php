                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between bg-galaxy-transparent">
                                <h4 class="mb-sm-0">Müşteri Aktar |  <?= $odeme['musteri_adi'] ?></h4>

                                <div class="page-title-right">
                                    <ol class="breadcrumb m-0">
                                        <li class="breadcrumb-item"><a href="javascript: void(0);">Nextario</a></li>
                                        <li class="breadcrumb-item active">Müşteri Aktar</li>
                                    </ol>
                                </div>
                        ...
                        <div class="col-lg-12">
                            <div class="card">
                                <div class="card-header">
                                    <h5 class="card-title mb-0">Müşteri Aktar</h5>
                                </div>
                                <div class="card-body">
                              <form action="islem.php?islem=musteri_aktar" method="POST"
      onsubmit="return confirm('Tüm kayıtlar aktarılacak. Emin misiniz?')">
         ...
         <div class="row">
         <div class="col-6">
        <label class="form-label">1. Pazarlamacı (ESKİ)</label>
        <select name="eski_pazarlamaci" class="form-select" required>
            <option value="">Seçiniz</option>
            <?php foreach ($users as $u): ?>
                <option value="<?= $u['id'] ?>">
                    <?= htmlspecialchars($u['username']) ?> - <?= htmlspecialchars($u['namesurname']) ?>
                </option>
            <?php endforeach; ?>
        </select>
        <small>Bu pazarlamacıya ait TÜM kayıtlar aktarılır.</small>
    </div>

    <div class="col-6">
        <label class="form-label">2. Pazarlamacı (YENİ)</label>
        <select name="yeni_pazarlamaci" class="form-select" required>
            <option value="">Seçiniz</option>
            <?php foreach ($users as $u): ?>
                <option value="<?= $u['id'] ?>">
                    <?= htmlspecialchars($u['username']) ?> - <?= htmlspecialchars($u['namesurname']) ?>
...
<script>
alert("<?= $_SESSION['mesaj']=='ok' ? 'Aktarım başarılı' : 'Hata oluştu' ?>");
</script>

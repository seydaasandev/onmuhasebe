...
$pazarlamaci_id = (int)$_POST['pazarlamaci_id'];
...
$stmt = $db->prepare("
    ...
    FROM musteriler
    WHERE sorumlu = ? AND durum = 0
    ORDER BY musteri_adi ASC
");
$stmt->execute([$pazarlamaci_id]);
...
foreach ($musteriler as $musteri) {
    ...
    $stmtS = $db->prepare("
        ...
        ORDER BY tarih DESC
    ");
    $stmtS->execute([$pazarlamaci_id, $m_id, $baslangic, $bitis]);
    $tum_satislar = $stmtS->fetchAll(PDO::FETCH_ASSOC);

    /* ======================
       TÜM ÖDEMELER (HESAPLAMA İÇİN)
    ====================== */
    $stmtO = $db->prepare("
        ...
        ORDER BY tarih ASC
    ");
    $stmtO->execute([$pazarlamaci_id, $m_id, $baslangic, $bitis]);
...
<head>
     ...
     <meta charset="utf-8" />
    <title>Pazarlamacı Ekstre Tablosu | Nextario Muhasebe Programı</title>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between bg-galaxy-transparent">
                                <h4 class="mb-sm-0">Pazarlamacı Ekstre Tablosu</h4>
                        ...
                        <div class="col-lg-12">
                            <div class="card">
                                <div class="card-header">
                                    <h5 class="card-title mb-0">Pazarlamacı Ekstre Tablosu</h5>
                                </div>
                                <div class="card-body">
                            <?php foreach($ekstre_musteriler as $m): ?>
<h4><?= htmlspecialchars($m['musteri_adi']) ?></h4>
<div class="alert alert-info">
    <b>Toplam Satış:</b> <?= number_format($m['toplam_satis'],2,',','.') ?> ₺<br>
    <b>Toplam Ödeme:</b> <?= number_format($m['toplam_odeme'],2,',','.') ?> ₺<br>
    <b>Kalan Borç:</b> <?= number_format($m['donem_borcu'],2,',','.') ?> ₺
</div>
<?php
$oran = $m['toplam_satis'] > 0 
    ? ($m['donem_borcu'] / $m['toplam_satis']) * 100 
    : 0;
...
<div class="mb-3">
    <small class="text-muted">Kalan Borç Oranı</small>
    <div class="progress" style="height:18px;">
        <div class="progress-bar <?= $barClass ?>"
             role="progressbar"
             style="width: <?= round($oran) ?>%;">
            <?= round($oran) ?>%
        </div>
    </div>
</div>

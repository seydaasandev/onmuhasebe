...
$stmt = $db->prepare("
    ...
    LEFT JOIN urunler p ON p.id = s.urun_id WHERE s.durum = 0 AND s.siparis = 1
    ORDER BY s.id ASC
");
$stmt->execute();
...
<head>
     ...
     <meta charset="utf-8" />
    <title>Siparişler | Nextario Muhasebe Programı</title>
...
<?php require "head.php"; ?>
    ...
    <tbody>
        ...
        <tr>
            <td><?= $s['id'] ?></td>
            <td><?= htmlspecialchars($s['islem_no']) ?></td>
            <td>
                <a href="musteri-hesap.php?id=<?= $s['musteri_id'] ?>" class="link-primary">
                    <?= htmlspecialchars($s['musteri_adi']) ?>
                </a>
            </td>
            <td><?= htmlspecialchars($s['urun_adi']) ?></td>
            <td class="text-end"><?= $s['adet'] ?></td>
            <td class="text-end"><?= number_format($s['kdv_toplami'],2,',','.') ?> ₺</td>
            <td class="text-end"><?= number_format($s['indirim_toplami'],2,',','.') ?> ₺</td>
            <td class="text-end"><?= number_format($s['tutar'],2,',','.') ?> ₺</td>
            <td><?= htmlspecialchars($s['satisi_yapan_adi']) ?></td>
            <td><?= date("d.m.Y H:i", strtotime($s['tarih'])) ?></td>
            <td><a href="#" class="btn btn-sm btn-danger satisSilBtn" data-id="<?= $s['id'] ?>"> Sil </a> </td>
        </tr>
        <?php endforeach; ?>
    </tbody>
</table>
